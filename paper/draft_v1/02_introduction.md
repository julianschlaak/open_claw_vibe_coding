...
1. **No distributional assumptions**: Empirical ranking works for any variable regardless of its statistical distribution.
2. **Seasonal stratification**: By comparing each day only to its day-of-year climatology, seasonal cycles are naturally accounted for without detrending.
3. **Interpretability**: Percentiles directly communicate rarity (e.g., "5th percentile" = "drier than 95% of historical observations").
4. **Comparability**: Different variables (soil moisture, recharge, streamflow) can be compared on a common 0–100 scale.

Despite these advantages, percentile-based multi-component indices remain underexplored in drought research.
